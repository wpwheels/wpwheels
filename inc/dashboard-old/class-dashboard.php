...
if ( ! class_exists( 'WPWheels_Dashboard' ) ) {
	...
	class WPWheels_Dashboard {
		...
		public function starter_template_button() {
			...
			foreach ( $this->get_wpwheels_extra_status() as $plugin_data ) :

				if ( 'active' === $plugin_data['status'] ) :
					$data_action = '';
				else :
					$plugins_active = false;
					$data_action = 'install_activate';
				endif;

				$data_slug = $plugin_data['slug'];

				$btn_html .= "<button type='button' class='button button-primary' data-action='" . esc_attr( $data_action ) . "' data-slug='" . esc_attr( $data_slug ) . "'></button>";
			endforeach;
